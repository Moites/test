...
class ModelDash:
    ...
    def create_dash(self):
        ...
        def update_dash(region, time_of_day):
            ...
            points_df = pd.read_sql_query('SELECT * FROM track_point', conn)
            filtered_df = pd.DataFrame.merge(track_df, metadata_df, on='track_id', how='inner')
            filtered_df.rename(columns={'elevation': 'track_elevation',
                                        'temperature': 'track_temperature'}, inplace=True)
            points_df_classificator = pd.DataFrame.merge(points_df, filtered_df, on='track_id', how='inner')
            conn.commit()
            filtered_df['track_datetime'] = pd.to_datetime(filtered_df['track_datetime'])
            ...
            temp_time_of_day = filtered_df.groupby('time_of_day')['track_temperature'].mean().reset_index()
            fig2 = px.bar(temp_time_of_day, x='time_of_day', y='track_temperature',
                          labels={'time_of_day': 'Время суток', 'track_temperature': 'Температура'},
                          color='time_of_day')
            ...
            fig4 = px.scatter(filtered_df, x='track_elevation', y='step_frequency',
                              labels={'track_elevation': 'Высота', 'step_frequency': 'Частота шагов'},
                              color='step_frequency')

            fig5 = px.scatter(filtered_df, x='track_elevation', y='track_temperature',
                              labels={'track_elevation': 'Высота', 'track_temperature': 'Температура'},
                              color='track_temperature')
            ...
            points_df_classificator['risk_level'] = points_df_classificator.apply(self.get_risk_level, axis=1)
            points_df_classificator['evac_level'] = points_df_classificator.apply(self.get_evacuation_level, axis=1)

            fig9 = px.scatter_map(points_df_classificator, lat='latitude', lon='longitude',
